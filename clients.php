...
<table>
    <thead>
        <tr>
            <th>Email</th>
            <th>Pseudo</th>
            <th>Capital déposé</th>
            <th>Date d'entrée</th>
            <th>Date de sortie</th>
            <th>Statut</th>
            <th>Performance (%)</th>
            <th>Capital + bénéfices ($)</th>
            <th>Montant final ($)</th>
            <th>Pénalité</th>
    ...
    <tbody>
        <?php if (empty($clients)): ?>
            <tr><td colspan="12">Aucun client trouvé.</td></tr>
        <?php else: ?>
            <?php foreach ($clients as $client): ?>
                <tr>
                    ...
                    <td><?= $client['statut'] ?></td>
                    <td><?= isset($client['performance']) ? number_format($client['performance'], 2) . '%' : '-' ?></td>
                    <td><?= isset($client['capital_benefices']) ? number_format($client['capital_benefices'], 2) . ' $' : '-' ?></td>
                    <td><?= isset($client['montant_final']) ? number_format($client['montant_final'], 2) . ' $' : '-' ?></td>
                    <td><?= $client['penalite_appliquee'] === 'oui' ? 'Oui' : 'Non' ?></td>
                    <td><?= htmlspecialchars($client['numero_contrat'] ?? '-') ?></td>
                    <td class="actions">
                        ...
                        <?php endif; ?>
                        <a class="btn delete" href="supprimer_client.php?id=<?= $client['id'] ?>" onclick="return confirm('❗ Cette action est irréversible.\n\nConfirmer la suppression du client : <?= htmlspecialchars($client['email']) ?> ?')">Supprimer</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        <?php endif; ?>
